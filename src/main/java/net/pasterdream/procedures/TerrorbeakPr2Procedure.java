...
public class TerrorbeakPr2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!(entity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(PasterdreamModMobEffects.SHADOW_SILENCE_BUFF.get()))) {
			if (new Object() {
				...
				if (entity instanceof TerrorbeakEntity) {
					((TerrorbeakEntity) entity).setAnimation("roar");
				}
				if (entity instanceof WeakenessTerrorbeakEntity) {
					((WeakenessTerrorbeakEntity) entity).setAnimation("roar");
				}
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pasterdream:terrorbeak_roar")), SoundSource.MASTER, (float) 0.6, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("pasterdream:terrorbeak_roar")), SoundSource.MASTER, (float) 0.6, 1, false);
					}
				}
				{
					...
					for (Entity entityiterator : _entfound) {
						if (!(entityiterator instanceof ItemEntity) && !(entityiterator instanceof TerrorbeakEntity)) {
